define(["THREE", "ModelGenerator/PerlinNoise", "TREE", "NURBS", "ModelGenerator/Utils"], function(THREE, noise, TREE, NURBS, Utils){
    ...
    function Flower(params){
        ...
        var makeSpiral = args.makeSpiral ? true : false;
        var flowerGeo;
        ...
        this.makeToolPath = function(){
            ...
            for(var i = 0 ; i < this.layers ; i++){
                ...
                this.cells.off = i*(this.layerHeight/this.scale);
            ...
            if(!makeSpiral){
                ...
                //make an array of non spiraled contour vertices
                var verts = [];
                for(var j = 0 ; j < flowerGeo.children.length ; j++){
                    for(var u = 0 ; u < flowerGeo.children[j].children.length ; u++){
                        for(var k = 0 ; k < flowerGeo.children[j].children[u].geometry.vertices.length ; k++){
                            var v = flowerGeo.children[j].children[u].geometry.vertices[k];
                            if(k==0)
                                v.e = 1;
                            else if(k==flowerGeo.children[j].children[u].geometry.vertices.length-1)
                                v.e = 2;
                            else
                                v.e = 0;
                            verts.push(v);
        ...
        this.turtle = function(){
            ...
            for(var i = 0 ; i < spiral.geometry.vertices.length-1 ; i++){
                ...
                var zero = new THREE.Vector3(0,0,1+(data.var4*10));
                var nPos = new THREE.Vector3(obj.position.x,obj.position.y,-1);
                var angler = nPos.angleTo(zero);
                // console.log(angler);
                kid.position.x = Math.cos((angler)*Math.ceil((.1+this.userData.tpPetals*Math.PI*4)*20))*this.userData.tpMult*.1;
                kid.position.z = Math.sin((angler)*Math.ceil((.1+this.userData.tpPetals*Math.PI*4)*20))*this.userData.tpLoop*.1;//THREE.Math.mapLinear(this.userData.tpPetals,0,.3,.26,.03);
        ...
        this.animateBalls = function(t){
            ...
            this.cells.off = tCount*(this.layerHeight/this.scale);
        ...
        this._setStart = function(a,b){
            ...
            if(a.vertices.length>0){
                ...
                for(var j = a.vertices.length-1 ; j >= 0; j--){               
                    ...
                    if (Math.abs(a.vertices[j].distanceTo(b)) < min){//.y-b.y)===0 && Math.abs(a.vertices[j].x-b.x)<min){

                        min = Math.abs(a.vertices[j].distanceTo(b));
                        t=j;
                    }
                }
        ...
        this.drawCanvas = function(v){
            ...
            if(this.canvas!==null){
                ...
                // ctx.globalBlendOperation = 'screen';
                ...
                function nGon(s,ox,oy,r,rot){
                    ...
                    for(var i = 1 ; i <= s ; i++){
                        ...
                        var x =  ox+Math.sin(rot+(i/s)*Math.PI*2)*e;
                        var y =  oy+Math.cos(rot+(i/s)*Math.PI*2)*e;
                        var px = ox+Math.sin(rot+((i-1)/s)*Math.PI*2)*e;
                        var py = oy+Math.cos(rot+((i-1)/s)*Math.PI*2)*e;
                        ...
                        ctx.fillStyle = gradient([ox,oy, cx, cy]);
                ...
                for(var i = 0 ; i < this.balls.balls.length ; i ++){
                    ...
                    var rt = this.balls.balls[i].rotation.z;
                    // console.log(rt);
                    ...
                    if(i>1){
                        ...
                        grd.addColorStop(0, 'rgba(128,0,0,128)');
                     ...
                     nGon(3+(Math.floor(data.var3*(1+data.var3)*10)), h+b.x*f, h+b.y*f,r*f,rt);
                    // this.counter++;
                    // if(i===0)
                    // nGon(3+(Math.floor(data.var3*(1+data.var3)*10)), h+noise(this.counter*.01)*f, h+noise(this.counter*.014)*f,1*f,rt);
        ...
        this._updateBallPosition = function(){
            ...
            for(i in b){
                for(j in b[i]){
                    ...
                    q.rotation.z = this.count * this.userData.baseTwist *  -.01;
                    q.parent = this;
    ...
    function saveGCode(arr,scalar) {
        ...
        var output = ";FLAVOR:UltiGCode\n;TIME:1081      \n;MATERIAL:1177      \n;MATERIAL2:0         \n\n;Layer count: 170\n;LAYER:0\nM107\nG1 F2000 X20.360 Y20.859 Z0.300 E-2.0\n;TYPE:WALL-OUTER\n";
